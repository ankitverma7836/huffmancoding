    ...
    return (
        <div className="decompress-page">
            ...
            <div className="decompress-content">
                ...
                <p>Select a Huffman encoded binary file to decompress it!</p> {/* Added instruction */}
                <input type="file" onChange={handleFileChange} />
                <p>{selectedFile ? selectedFile.name : 'No file chosen'}</p>
                <button onClick={handleFileUpload} disabled={!selectedFile}>
                    Decompress and Download
                </button>
                <p>Please wait after uploading so that your file can be decompressed.</p> {/* Added loading instruction */}
            </div>
        </div>
    );
};
